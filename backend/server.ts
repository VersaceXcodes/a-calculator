...
app.post('/api/calculations', authenticateToken, async (req, res) => {
  try {
    // Validate request body
    const validatedData = createCalculationInputSchema.parse(req.body);
    ...
    try {
      ...
      res.status(201).json({
        ...
        calculation: {
          ...
          created_at: new Date(calculation.created_at),
          updated_at: new Date(calculation.updated_at),
...
/*
  ...
  Retrieves calculation details for authenticated users
*/
    ...
    try {
      ...
      res.json({
        calculation: {
          ...
          result: calculation.result,
          created_at: new Date(calculation.created_at),
          updated_at: calculation.updated_at ? new Date(calculation.updated_at) : null,
          device_type: calculation.device_type,
          device_id: calculation.device_id,
          error_message: calculation.error_message,
          user_agent: calculation.user_agent
...
app.post('/api/settings', authenticateToken, async (req, res) => {
  try {
    ...
    try {
      ...
      const existingSettings = await client.query('SELECT setting_id FROM settings WHERE user_id = $1', [validatedData.user_id]);
    ...
    try {
      ...
      res.json({
        setting: {
          ...
          button_size: setting.button_size,
          created_at: new Date(setting.created_at),
          updated_at: setting.updated_at ? new Date(setting.updated_at) : null
        }
      });
    } finally {
      client.release();
...
app.put('/api/settings/:setting_id', authenticateToken, async (req, res) => {
  try {
    ...
    try {
      ...
      const existingSettings = await client.query('SELECT * FROM settings WHERE setting_id = $1', [setting_id]);
      if (existingSettings.rows.length === 0) {
        return res.status(404).json(createErrorResponse('Settings not found', null, 'SETTINGS_NOT_FOUND'));
      }
